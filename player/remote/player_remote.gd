...
const NECK_ROT_SCALER: PackedFloat32Array = [0.2, -0.2]
const ABDOMEN_ROT_SCALER: PackedFloat32Array = [0.17, 0]
const TORSO_ROT_SCALER: PackedFloat32Array = [0.19, -0.15]
const ARM_X_ROT_SCALER: PackedFloat32Array = [0.19, 0]
const ARM_Y_ROT_SCALER: PackedFloat32Array = [-0.39, -1.25]
...
func set_rot_x_visuals(rot_x: float) -> void:
	...
	q_neck.x = lerp_angle(NECK_ROT_SCALER[0], NECK_ROT_SCALER[1], rot_weight)
	skeleton.set_bone_pose_rotation(bone_neck, q_neck)

	#Torso
	var q_torso := skeleton.get_bone_pose_rotation(bone_torso)
	q_torso.x = lerp_angle(TORSO_ROT_SCALER[0], TORSO_ROT_SCALER[1], rot_weight)
	skeleton.set_bone_pose_rotation(bone_torso, q_torso)
	
	#Abdomen
	var q_abdomen := skeleton.get_bone_pose_rotation(bone_abdomen)
	q_abdomen.x = lerp_angle(ABDOMEN_ROT_SCALER[0], ABDOMEN_ROT_SCALER[1], rot_weight)
	skeleton.set_bone_pose_rotation(bone_abdomen, q_abdomen)
	
	#Upper arm Right
	var q_upper_arm_r := skeleton.get_bone_pose_rotation(bone_upper_arm_r)
	q_upper_arm_r.x = lerp_angle(ARM_X_ROT_SCALER[0], ARM_X_ROT_SCALER[1], rot_weight)
	q_upper_arm_r.y = lerp_angle(ARM_Y_ROT_SCALER[0], ARM_Y_ROT_SCALER[1], rot_weight)
